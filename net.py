...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(135424, 512)#25600 135424
        self.fc2 = nn.Linear(512, 64)
        self.fc3 = nn.Linear(64, 9)
        self.fcbb1 = nn.Linear(512, 4*9*5)
        self.fcbb2 = nn.Linear(4*9*5, 4*9)
...
def test(model, test_loader):
    ...
    with torch.no_grad():
        for data, target, bounding_box in test_loader:
            outcol, outcoor = model(data.to("cuda"))
            target = target.to("cuda")
            bounding_box = bounding_box.to("cuda")
            # crit = torch.nn.BCEWithLogitsLoss(pos_weight=torch.add(torch.flatten(target), 1))
            critcol = torch.nn.BCEWithLogitsLoss()
            test_loss_col += critcol(torch.flatten(outcol), torch.flatten(target).float()).item()  # sum up batch loss

            crit_coor = torch.nn.BCEWithLogitsLoss()
            test_loss_coor += critcol(torch.flatten(outcoor), torch.flatten(bounding_box).float()).item()
            ...
            correct_coor += (vector_to_check.sum(dim=1)).sum().item()/(1200)
            # print(correct_coor)
